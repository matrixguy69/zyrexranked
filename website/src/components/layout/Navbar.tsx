        ...
        <div className="flex items-center justify-between h-16">
            ...
            {links.map(link => (
              <Link key={link.href} href={link.href}
                className={clsx('px-3 py-1.5 rounded-lg text-sm font-medium transition-colors',
                  pathname === link.href ? 'text-white bg-brand-primary/20' : 'text-slate-400 hover:text-white hover:bg-white/5')}>
                {link.label}
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 bg-dark-500/60 border border-white/10 rounded-lg px-3 py-1.5">
              ...
              <span className="text-xs text-slate-400 font-mono">zyrex.xyz</span>
            </div>
            <button className="md:hidden p-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5" onClick={() => setMobileOpen(!mobileOpen)}>
