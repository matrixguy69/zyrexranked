...
import { Trophy, Sword, TrendingUp, Clock, Star, Flame, Shield, ArrowLeft } from 'lucide-react';
import clsx from 'clsx';
import {
  GAMEMODES, getRank, getRankClass, formatElo, formatWL,
  timeAgo, formatDuration, eloProgress, eloToNextRank
} from '@/lib/utils';
...
export default function PlayerPage() {
  ...
  const [activeGm, setActiveGm] = useState('global');
  ...
  if (loading) return (
    <div className="max-w-4xl mx-auto px-4 py-20 text-center text-slate-500 animate-pulse">
      Loading player...
    </div>
  );
  ...
  if (notFound || !player) return (
    <div className="max-w-4xl mx-auto px-4 py-20 text-center">
      ...
      <Link href="/leaderboard" className="text-brand-accent hover:text-brand-primary">
        ← Back to Leaderboard
      </Link>
    </div>
  );
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
          ...
          <div className="relative">
            <img
              ...
              onError={(e) => {
                (e.target as HTMLImageElement).src = player.avatarUrl;
              }}
            />
            <div
              className="absolute -bottom-1.5 -right-1.5 w-5 h-5 rounded-full border-2 border-dark-600"
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-sm text-slate-500 flex-wrap">
              ...
              <span className="flex items-center gap-1">
                <Star size={12} /> Joined {new Date(player.createdAt).toLocaleDateString()}
              </span>
            </div>
          </div>
          ...
          <div className="text-right sm:text-right">
            ...
            <div className="text-4xl font-black" style={{ color: globalRank.color }}>
              {formatElo(player.globalElo)}
            </div>
            <div className="text-xs text-slate-500 mt-1">
              {player.totalWins}W &nbsp;{player.totalLosses}L &nbsp;
              <span className="text-slate-400">{formatWL(player.totalWins, player.totalLosses)} K/D</span>
            </div>
          </div>
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                activeGm === gm.id
                  ? 'text-white border'
                  : 'text-slate-400 hover:text-white bg-dark-500/40'
              )}
              style={activeGm === gm.id ? {
            ...
            {player.recentMatches.map((match: any) => {
              ...
              return (
                <div key={match.matchId} className="flex items-center gap-4 px-5 py-3.5 hover:bg-dark-400/20 transition-colors">
                  {/* Result badge */}
                  <div className={clsx(
                    'w-12 text-center text-xs font-bold py-1.5 rounded-lg flex-shrink-0',
                    isWin ? 'bg-emerald-500/15 text-emerald-400' :
                    isDraw ? 'bg-yellow-500/15 text-yellow-400' :
                    isCancelled ? 'bg-gray-500/15 text-gray-400' :
                  ...
                  <div className="hidden sm:block flex-shrink-0">
                    <span className="text-xs px-2 py-1 rounded-md bg-dark-400/40 text-slate-400">
                      {gm?.emoji} {gm?.label || match.gamemode}
                    </span>
                  </div>
